...
using MyPassword.Migrations.SeedData;
...
namespace MyPassword.Tests
{
    public abstract class MyPasswordTestBase : AbpIntegratedTestBase<MyPasswordTestModule>
    {
        ...
        protected void UsingDbContext(Action<MyPasswordDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MyPasswordDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MyPasswordDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MyPasswordDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MyPasswordDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyPasswordDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MyPasswordDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MyPasswordDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MyPasswordDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MyPasswordDbContext, Task<T>> func)
        {
            T result;
